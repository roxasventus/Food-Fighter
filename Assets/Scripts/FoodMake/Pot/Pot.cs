    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        if (mouseHand.Gethand() == null)
        {
            if (s == CooKingDatac.Status.incomplete || s == CooKingDatac.Status.complete ) 
            {
                ...
                if (cooKData.foodbase == FoodBase.noodle) { recipe.SetBase(Recipe.Base.noodle); }else
                if (cooKData.foodbase == FoodBase.tteok) { recipe.SetBase(Recipe.Base.tteok); }

                if (cooKData.status == CooKingDatac.Status.incomplete) { recipe.SetStatus(Recipe.Status.incomplete); }else
                if (cooKData.status == CooKingDatac.Status.complete) { recipe.SetStatus(Recipe.Status.complete); }

                if (cooKData.sauce == CooKingDatac.Sauce.soup) {  recipe.SetSauce(Recipe.Sauce.soup); }else
                if (cooKData.sauce == CooKingDatac.Sauce.jjajang) { recipe.SetSauce(Recipe.Sauce.jjajang); }

                if (cooKData.special == CooKingDatac.Special.miwon) { recipe.SetSpecial(Recipe.Special.miwon); }else
                if (cooKData.special == CooKingDatac.Special.olive) { recipe.SetSpecial(Recipe.Special.olive); }else
